...
Singleton {
    ...
    sizes: QtObject {
        function getBaseBarHeight(w) {
            if (w >= 3600) return 60;
            if (w >= 2560) return 46;
            return 40;
        }
        function getBarHeight(w) {
            return Config.options.bar.cornerStyle === 1 ? 
                (getBaseBarHeight(w) + getHyprlandGapsOut(w) * 2) : getBaseBarHeight(w);
        }
